...
class Question4Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view : View =  inflater.inflate(R.layout.fragment_question4, container, false)

        val radioq : RadioGroup = view.findViewById<RadioGroup>(R.id.rg_question4)
        ...
        { radioGroup: RadioGroup, i: Int ->
            ...
            Global.AnswerQuestion[3]=true;


            radiocheck= radioButton.text.toString().substring(0,1)
            Global.UserResponses[3]=radiocheck

            val sharedPreferences =
                activity?.getSharedPreferences(APP_QUESTION_4, Context.MODE_PRIVATE)

            if (sharedPreferences != null) {
                with(sharedPreferences.edit()) {
                    putString(APP_QUESTION_4, radiocheck)
                    apply()
                }
            }
